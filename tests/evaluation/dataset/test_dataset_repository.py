...
from pharia_studio_sdk.evaluation.dataset.hugging_face_dataset_repository import (
...
def mocked_hugging_face_dataset_repository(
    ...
    class_to_patch = "pharia_studio_sdk.evaluation.dataset.hugging_face_dataset_repository.HuggingFaceDatasetRepository"
    with (
...
@patch(target="pharia_studio_sdk.evaluation.dataset.domain.uuid4", return_value="1234")
@mark.parametrize(
    "repository_fixture",
    test_repository_fixtures,
)
...
@patch(
    target="pharia_studio_sdk.evaluation.dataset.file_dataset_repository.LocalFileSystem.exists",
    return_value=True,
)
